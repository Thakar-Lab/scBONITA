...
def readKEGG(lines, graph, KEGGdict, hsaDict):
    ...
    for relation in soup.find_all('relation'):
        ...
        if ('activation' in subtypes) or ('expression' in subtypes) or ('glycosylation' in subtypes):
            color='green'
            signal='a'
...
def find_pathways_kegg(geneList, preDefList = [], writeGraphml=True,  organism="hsa", minimumOverlap = 1):
    ...
    for x in pathwayList:
        x=x.replace("path:","")
        code=str(x)
        code=re.sub("[a-zA-Z]+", "", code) # eliminate org letters - retain only numbers from KEGG pathway codes
        origCode = code
        ...
        for code in [coder]:
            print(code)
            try:
                url = requests.get("http://rest.kegg.jp/get/"+code+"/kgml", stream=True)
        ...
        if test > minimumOverlap and len(graph.edges()) > 0: # if there are at least minimumOverlap genes shared between the network and the genes in the dataset
