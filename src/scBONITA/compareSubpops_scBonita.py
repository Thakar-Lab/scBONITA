...
def compareSubpops(graphNames):
    ...
    for graphName in graphNames:
        ...
        if Path(dataName).is_file() and Path(graphName).is_file() and Path(equivs).is_file() and Path(graphName+"_processed.graphml_importanceScores.csv").is_file():
            ...
            allscData.inherit(allscData.pathwayGraphs[graphName], removeSelfEdges=False, restrictIncomingEdges=False)
            equivs=pickle.load(open(graphName+"_processed.graphml_equivs1.pickle", "rb"))
            ...
            minimalRules = processERS_minimalRule(graphName+"_processed.graphml_equivs1.pickle", allscData)
            plainEquivs = {}
            bitstringEquivs = {}
            minimalRulesNodes = {}
            for node in range(0,len(nodeList)):
                ...
                for rule in ers:
                    plainRules.append(allscData.writeNode(allscData.nodeList.index(allscData.nodeList[node]), rule, allscData))
                plainEquivs[nodeList[node]]=set(plainRules)
                bitstringEquivs[nodeList[node]]=set(tuple(ers[i]) for i in range(0, len(ers)))
                minimalRulesNodes[nodeList[node]]=allscData.writeNode(node,minimalRules[allscData.individualParse[node]:allscData.individualParse[node+1]],allscData)
            in_degree = [allscData.pathwayGraphs[graphName].in_degree(node) for node in nodeList]
            out_degree = [allscData.pathwayGraphs[graphName].out_degree(node) for node in nodeList]
            inOutRatio = [float(inDeg+1)/(outDeg+1) for inDeg, outDeg in zip(in_degree, out_degree)]
            taoScore = {key: np.nan for key in nodeList} #ruleScore(nx.read_graphml(graphName))

            #Get importance score
            importanceScoreDF = pd.read_csv(graphName+"_processed.graphml_importanceScores.csv")
            importanceScoreDict = {}
            ...
            tempdf = pd.DataFrame(list(zip(nodeList, in_degree, out_degree, inOutRatio, equivs_len)), columns =['Node','In_degree', 'Out_degree', 'inOutRatio', 'Equivs_len'])
...
def makePlots():
    ...
    outputDF.plot(x ='inOutRatio', y='importanceScore', kind = 'scatter', title='(In-degree + 1)/(Out-degree + 1) vs. Importance Score\nCorrelation: '+str(outputDF.corr()['inOutRatio']['importanceScore']))
    plt.xlabel('(In-degree + 1)/(Out-degree + 1)')
    plt.ylabel('Importance Score')
    plt.savefig('inOutRatio_vs_importanceScore.png')

    outputDF.plot(x ='Out_degree', y='importanceScore', kind = 'scatter', title='Out-degree vs. Importance Score\nCorrelation: '+str(outputDF.corr()['Out_degree']['importanceScore']))
    plt.xlabel('Out-degree')
    plt.ylabel('Importance Score')
    plt.savefig('outDegree_vs_importanceScore.png')

    outputDF.plot(x ='In_degree', y='importanceScore', kind = 'scatter', title='In-degree vs. Importance Score\nCorrelation: '+str(outputDF.corr()['In_degree']['importanceScore']))
    ...
    sourceNodesDF.plot(x ='inOutRatio', y='importanceScore', kind = 'scatter', title='For source nodes: (In-degree + 1)/(Out-degree + 1) vs. Importance Score\nCorrelation: '+str(sourceNodesDF.corr()['inOutRatio']['importanceScore']))
    plt.xlabel('(In-degree + 1)/(Out-degree + 1)')
    plt.ylabel('Importance Score')
    ...
    sns.set(style="ticks")
    tempPlot=sns.catplot(x="Equivs_len", y="Subpop", data=outputDF2,  orient="h", color="grey") #, kind="boxen"
    ...
    sns.set(style="ticks")
    tempPlot=sns.catplot(x="putativeERS", y="Subpop", data=outputDF2,  orient="h", color="grey") #, kind="boxen"
    ...
    for subpop1 in list(set(netDF.Subpop)):
        ...
        for subpop2 in list(set(netDF.Subpop)):
            ...
            for node in list(set(netDF.Node)):
                ...
                firstMatch = next((a for a in subpop1_equivs if a in subpop2_equivs), None)
                if firstMatch:
                    percentOverlap = 1
                    outputMatrix[subpop1][subpop2] = percentOverlap + outputMatrix[subpop1][subpop2]
                else:
...
def makeSubpopCompareHeatmaps():
    ...
    totalDF.columns = [subpopNames[subpopNumber] for subpopNumber in list(totalDF.columns)]
    totalDF.index = [subpopNames[subpopNumber] for subpopNumber in list(totalDF.index)]
    totalDF.to_csv("totalDF.csv")
    with sns.axes_style("white"):
        f, ax = plt.subplots(figsize=(11, 11))
        ax = sns.heatmap(totalDF, mask=mask, vmax=.3, square=True, linewidths=.3, annot=True, cmap="autumn_r", fmt='.1f')
    ...
    for resFile in myfiles:
        resDF = pd.read_csv(resFile, index_col=0)
        resDF.columns = [subpopNames[subpopNumber] for subpopNumber in list(resDF.columns)]
        ...
        with sns.axes_style("white"):
            f, ax = plt.subplots(figsize=(11, 11))
            ax = sns.heatmap(resDF, mask=mask, vmax=.3, square=True, linewidths=.3, annot=True, cmap="autumn_r", fmt='.1f')
            #ax.set_xticklabels(ax.get_xmajorticklabels(), fontsize = 10)
            #ax.set_yticklabels(ax.get_xmajorticklabels(), fontsize = 10)
            title = str(pathName+'\n').upper()
            plt.title(title, loc="left")
        ...
        if os.path.exists(network):
            ...
            if len(set(clusterLabels.decider)) > 1:
                ...
                plt.scatter(
                    ...
                    s = 0.1)
                plt.gca().set_aspect('equal', 'datalim')
                plt.colorbar(boundaries=np.arange(len(set(labeledData.decider)))-0.5).set_ticks(np.arange(len(set(labeledData.decider))))
                plt.title(str(networkName), fontsize=12)
                plt.savefig('bonita_labels_'+orig_network+".pdf", bbox_inches='tight')
                plt.close()
                #pdf.savefig()
    #pdf.close()
...
if __name__ == '__main__':
    ...
    subpopNames = {"11_subpop" : "T cells CD8/NK cells resting","14_subpop" : "Monocytes"} #{"0_subpop" : "B cells naive - 1", "1_subpop" : "T cells CD8 - 1", "2_subpop" : "T cells CD4 - 1", "3_subpop" : "T cells CD8/CD4/CD4 naive", "4_subpop" : "T cells CD8 naive/T cells CD8/ RBCs", "5_subpop" : "B cells naive - 2", "6_subpop" : "T cells naive", "7_subpop" : "T cells CD8 - 2", "8_subpop" : "T cells CD8 - 3", "9_subpop" : "NK cells resting", "10_subpop"  : "T cells CD8 - 4", "11_subpop" : "T cells CD8/NK cells resting", "12_subpop" : "T cells CD4 memory resting/ T cells CD8", "13_subpop" : "B cells memory", "14_subpop" : "Monocytes", "15_subpop" : "NK cells activated"}
    paths = glob.glob('*/')
    mainDir = os.getcwd()
    for subpop in paths:
        if str(subpop)[:-1] in subpopNames.keys():
            ...
            print(ruleFile)
            rule_analysis_file = pd.read_csv(ruleFile[0]) #these files come from compareSubpops_scBonita.py
            networksOfInterest = ["hsa04024.graphml", "hsa04630.graphml"]#glob.glob("*.graphml")#["hsa04151.graphml", "hsa04024.graphml", "hsa04670.graphml", "hsa04630.graphml", "hsa04066.graphml", "hsa04933.graphml", "hsa04810.graphml","hsa04360.graphml","hsa04218.graphml","hsa04360.graphml"] 
            for network in networksOfInterest:
                if os.path.exists(network[0:8]+"_IS_AS_vs_nonAS.graphml"):
                    print(network)
                    rule_analysis_network = rule_analysis_file.loc[rule_analysis_file.Graph == network, :]
                    rule_analysis_network.loc[:,"plainEquivs"] = rule_analysis_network.plainEquivs.str.replace(pat = "{", repl = "[", regex = True)
                    rule_analysis_network.loc[:,"plainEquivs"] = rule_analysis_network.plainEquivs.str.replace(pat = "}", repl = "]", regex = True)
                    rule_analysis_network.loc[:,"plainEquivs"] = [literal_eval(temp) for temp in rule_analysis_network.plainEquivs.tolist()]
                    sampledRules = [choice(temp) for temp in rule_analysis_network.plainEquivs.tolist()]
                    ruleGraph = Get_expanded_network(sampledRules,equal_sign='*=')
                    ruleGraph = nx.relabel_nodes(ruleGraph, lambda x: x.replace(" ", ""))
                    original = nx.read_graphml(network[0:8]+"_IS_AS_vs_nonAS.graphml")
                    nx.set_node_attributes(original,name='Display Name', values={k : k.replace(" ", "") for k in original.nodes()})
                    nx.set_node_attributes(original,name='andNode',values={k: 0 for k in original.nodes()})
                    RAval = nx.get_node_attributes(original, "relativeAbundance")
                    nx.set_node_attributes(ruleGraph,name='relativeAbundance',values={k: RAval[k] if k in list(original.nodes()) else 0. for k in list(ruleGraph.nodes())})
                    pathImportances = nx.get_node_attributes(original, "Strat3_IS")
                    print(pathImportances)
                    nx.set_node_attributes(ruleGraph,name='importanceScore', values={k: pathImportances[k] if k in list(original.nodes()) else 0. for k in list(ruleGraph.nodes())})
                    DisplayName = nx.get_node_attributes(original, "Display Name")
                    nx.set_node_attributes(ruleGraph,name='Display Name', values={k:DisplayName[k] if k in list(original.nodes()) else "&" for k in list(ruleGraph.nodes())})
                    nx.write_graphml_lxml(ruleGraph, str(subpop)[:-1]+"_"+network[0:8]+"_ruleGraph.graphml")
            os.chdir(mainDir)
