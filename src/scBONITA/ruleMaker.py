...
class ruleMaker:
    ...
    def __init__(self, graph, removeSelfEdges=False, restrictIncomingEdges=True, maxIncomingEdges=3, groundTruth=False, graphName=""):
        ...
        self.nodePositions = [self.geneList.index(node) for node in nodeList if node in self.geneList] # node positions in geneList
    ...
    def eaMuPlusLambdaAdaptive(self, scSyncBoolC, graph, verbose=True):
        ...
        fitnesses=[indy[0].evaluateByNode(indy[1], KOlist, KIlist,scSyncBoolC) for indy in invalid_ind]
    ...
    def evaluateByNode(self, individual, KOlist, KIlist,cFunction, localSearch=False, importanceScores=False):
        ...
        simSteps=self.params.simSteps
        lenSamples1=len(model.sampleList)
        binMatC3=np.array(copy.deepcopy(self.binMat.toarray(order="C")), order="C",dtype=np.intc)
        binMatCPointer=ctypes.c_void_p(binMatC3.ctypes.data) # put input array as C pointer
        ...
        vals=np.full(shape=(100, self.maxNodes), fill_value=0, dtype=np.intc, order='C') #simData[STEP][NODE]
        ...
        if localSearch:
            ...
            errors = np.array(np.full(self.maxNodes, fill_value=0, dtype=np.intc, order='C'))
            errorsSubmit=ctypes.c_void_p(errors.ctypes.data)
            cFunction(valsubmit,nodeIndividual1, indLen1, nodeNum1, andLenList1, individualParse1, andNodes1, andNodeInvertList1, simSteps1, knockouts1, knockins1, lenSamples, binMatCPointer, nodePositionsCPointer, errorsSubmit, localSearchC, importanceScoresC) #in this case scSyncBoolC
            errors = errors.tolist()
            errors=errors[:nodeNum]
            return(errors)
        else:
            if importanceScores:
                importanceScores=np.array(np.full(1, fill_value=0.0, dtype=np.float64, order='C'))
                importanceScoresC=ctypes.c_void_p(importanceScores.ctypes.data)
                cFunction(valsubmit,nodeIndividual1, indLen1, nodeNum1, andLenList1, individualParse1, andNodes1, andNodeInvertList1, simSteps1, knockouts1, knockins1, lenSamples, binMatCPointer, nodePositionsCPointer, importanceScoresC) #in this case importanceScore
                return(importanceScores.tolist())
            else:
                #look at errors by sample
                errors = np.array(np.full(self.maxSamples, fill_value=0, dtype=np.intc, order='C'))
                errorsSubmit=ctypes.c_void_p(errors.ctypes.data)
                cFunction(valsubmit,nodeIndividual1, indLen1, nodeNum1, andLenList1, individualParse1, andNodes1, andNodeInvertList1, simSteps1, knockouts1, knockins1, lenSamples, binMatCPointer, nodePositionsCPointer, errorsSubmit, localSearchC, importanceScoresC) #in this case scSyncBoolC
                errors = errors.tolist()
                return([sum(errors)])
    ...
    def scoreNodes(self, graph):
        ...
        #print(self.geneList)
        #print(list(net))
        netGenes = [self.geneList.index(gene) for gene in list(net) if gene in self.geneList]
        print(netGenes)
        updateBooler=ctypes.cdll.LoadLibrary('./simulator.so')
        scSyncBoolC=updateBooler.scSyncBool
        importanceScore = updateBooler.importanceScore
        self.inherit(net, removeSelfEdges=False, restrictIncomingEdges=True, maxIncomingEdges=3, groundTruth=False)
        ...
        #Local search
        outputs=[self.checkNodePossibilities(node, out2, model.knockoutLists, model.knockinLists,scSyncBoolC) for node in range(0, len(model.nodePositions))]
        equivs=[]
        individual=[]
        devs=[]
        localErrors=[]
        ...
        importanceScores = self.calcImportance("", self, importanceScore, graph)
    ...
    def checkNodePossibilities(self, node, indy, KOlist, KIlist, scSyncBoolC):
        model=self
        ...
        for i in range(1,2**(end-start)):
            ...
            tempInd=model.bitList(i, len(truth))
            tempultimate[start:end]=tempInd # set rule to one being checked
            currentsumtemp=self.evaluateByNode(tempultimate,KOlist, KIlist, scSyncBoolC, localSearch=True)
    ...
    def calcImportance(self, equivs, model, importanceScore, graphName):
        ...
        i = 0
        while (i < 3):
            individual = self.processERS(graphName+"_equivs1.pickle")
            for node in tempList:
                print("Node: " + str(self.nodeList[node]) + ", Node Position: " + str(node))
                temp = self.evaluateByNode(individual, [node], [node], importanceScore, localSearch=False, importanceScores=True) 
        ...
        for node in range(0, len(self.nodeList)):
            importanceScoresDict[self.nodeList[node]] = (importanceScoresDict[self.nodeList[node]][0] - minScore[0])/(maxScore[0] - minScore[0])
        ...
        for node in range(0, len(self.nodeList)):
            ...
            if inDegree == 0:
                ...
                maxERS[self.nodeList[node]] = 2**(len(ers[node][0])) - 1 #2**(inDegree+1) - 1 #
            #print(node)
            ...
            importanceScoresDict[self.nodeList[node]] = importanceScoresDict[self.nodeList[node]] * ((maxERS[self.nodeList[node]] - obsERS[self.nodeList[node]] + 1)/maxERS[self.nodeList[node]])
            #importanceScoresDict[self.nodeList[node]] = np.mean(importanceScoresDict[self.nodeList[node]])
        ...
        IS_df = pd.DataFrame(importanceScoresDict.items(), columns=["Node", "Importance Score"])
        ...
        IS_df.to_csv(str(graphName + "_importanceScores.csv"), sep=',', encoding='utf-8', index=False)
    ...
    class Params:
        ...
        def simParams(self, mutModel=0.25, cells=1, samples=1, generations=5, popSize=24, mu=10, lambd=24, iters=100, genSteps=100, simSteps=100,crossoverProb=0.1, mutationProb=0.9, bitFlipProb=0.5):
