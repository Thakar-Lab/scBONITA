...
class singleCell(ruleMaker):
    ...
    def __init__(self, dataName, sep, maxNodes=15000, maxSamples=10000):
    ...
    def add_pathways(self, minOverlap=20, writeGraphml=True, removeSelfEdges=False):
        if hasattr(self, "cvGenes"):
            pathwayGenes=set(self.cvGenes)
        elif not hasattr(self, "cvGenes"):
            print("You have not filtered genes by any criterion.")
        ...
        for x in list(glob.glob('*.graphml')): #list(glob.glob('*[0-9].graphml')):
            G = nx.read_graphml(x)
            nodes=set(G.nodes())
            test=len(nodes.intersection(pathwayGenes))
    ...
    def run_pathway_analysis_from_outs(self, contrast, conditions, conditionsSep = "\t", contrastSep = "\t"):
                ...
                if os.path.exists(pathname[:-8]+"_IS.graphml") or os.path.exists(pathname[:-8]+".graphml_processed.graphml_importanceScores.csv"):
                    ...
                    if os.path.exists(pathname[:-8]+".graphml_processed.graphml_importanceScores.csv"):
                        ...
                        if "importanceScore" in nodeScoresDF.columns:
                            # add impact score as attribute to graph
                            nodeScores = dict(zip(list(nodeScoresDF.index), list(nodeScoresDF.loc[:, "importanceScore"])))
                            nx.set_node_attributes(paNetTemp, values=nodeScores, name='importanceScore')
    ...
    def processERS_minimalRule(self, equivsName):
        ...
        for i in range(len(ers)):
            ...
            maxOrRules = ruleSet[numOrRules.index(
                deciderVar)]  #rules with maximum or terms
            maxUpstreamNodes = 0
            minimalRule = []
            for orRule in [maxOrRules]:
                if sum(orRule) > 0:
                    numUpstreamNodes = [
                        ...
                        for orTerm in range(len(orRule)) if orRule[orTerm] == 1
                    ]
                else:
                    minimalRule = orRule
                    continue
    ...
    def updateBool(self, currentNode, oldValue, nodeIndividual, andNodes, andNodeInvertList, nodeStart, nodeEnd):
        indindex = nodeStart
        ...
        while indindex < nodeEnd:
            ...
            if (nodeIndividual[indindex]):
                newval = (oldValue[andNodes[andindex][0]] !=
                        andNodeInvertList[andindex][0])
                addnode = 0
                while addnode < len(andNodes[andindex]):
                    if (andNodes[andindex][addnode] > (-1)):
                        newval = (newval
                                and (oldValue[andNodes[andindex][addnode]] !=
    ...
    def getAttractCycle2(self, valsT):
    ...
    def getAttract(self, vals, resSubmit):
        res = self.getAttractCycle2(vals)
        resSubmit[0] = res[0]
        resSubmit[1] = res[1]
        return resSubmit

    def cluster(self, simData, resSubmit, sampleIndex, individual, indLen, nodeNum,
                andLenList, individualParse, andNodes, andNodeInvertList, simSteps,
                knockouts, knockins, binMat, nodePositions):
        gc.collect()
        ...
        while step < simSteps:
            ...
            while i < nodeNum:
                if (knockouts[i] == 1):
                    ...
                    temp = self.updateBool(i, oldValue, individual, andNodes[i],
                                    andNodeInvertList[i], nodeStart, nodeEnd)
                    newValue[i] = temp
                    nodePos = nodePositions[i]
                    simData[step][i] = temp
                i = i + 1
            step = step + 1
        gc.collect()
        resSubmit = self.getAttract(simData, resSubmit)
        gc.collect()
        return (resSubmit)
            ...
            if path.exists(str(pathway + "_equivs1.pickle")):
                ...
                if useMinimalRuleSet:
                    ...
                    individual =self.processERS(
                        str(pathway + "_equivs1.pickle"))  #get a rule set
                with open(pathway[0:8] + "_minimal_rules.txt", "w") as text_file:
                    text_file.write(self.writeModel(individual, self))
                text_file.close()
                knockins = np.zeros(len(self.nodeList),
                                    dtype=np.intc,
                                    order='C')  #set up empty ko and ki genes
                knockouts = np.zeros(len(self.nodeList),
                                    dtype=np.intc,
                                    order='C')
                ...
                individualParse = np.array(self.individualParse,
                                        dtype=np.intc,
                                        order='C')
                andLenList = np.array(self.andLenList,
                                    dtype=np.intc,
                                    order='C')
                nodePositions1 = self.nodePositions
                nodePositionsC = np.array(nodePositions1, dtype=np.intc, order="C")
                binMatC3 = np.array(copy.deepcopy(
                    self.binMat.toarray(order="C")),
                                    order="C",
                                    dtype=np.intc)
                for sampleIndex in range(0, len(self.sampleList)):
                    vals = np.full(shape=(simSteps, nodeNum),
                                fill_value=0,
                                dtype=np.intc,
                                order='C')
                    res = np.full(shape=2, fill_value=2, dtype=np.intc,
                                order='C')  # initiate output array
                    res = self.cluster(vals, res, sampleIndex, nodeIndividual, indLen,
                                nodeNum, andLenList, individualParse, andNodes,
                                andNodeInvert, simSteps, knockouts, knockins,
                                binMatC3, self.nodePositions)
                    attractor = []
                ...
                attractorList = [
                    list(x) for x in set(tuple(x) for x in attractorList)
                ]
                print(
                    str(len(attractorList)) +
                    " unique attractors were identified for " + pathway)
                #remove attractors that are all 0
                testIfZero = [sum(x) for x in set(tuple(x) for x in attractorList)]
                attractorList = [
                    attractorList[i] for i in range(len(attractorList))
                    if testIfZero[i] > 0
                ]
                ...
                for i in range(0, len(self.sampleList)):
                    ...
                    for attr in attractorList:
                        newdistanceDict[cell].append(
                            sum([
                                abs(i1 - i2[0])
                                for i1, i2 in zip(attr, cellInitValue)
                            ]))
                #Change table to a pandas dataframe for easy readability and plotting
                distanceDF = pd.DataFrame.from_dict(newdistanceDict,
                                                    orient='index')
                ...
                print(
                    str(len(decider)) +
                    " of these unique attractors mapped to cells for " + pathway)
                distanceDF.to_csv(str(pathway + "_attractorDistance.csv"),
                                index=True)
                return distanceDF

    def makeAttractorHeatmaps(self, distanceDF,
                          network,
                          width,
                          height,
                          allAttractors=False, cmap = "RdYlBu_r", numberOfAttractorsToShow = 2, outputFileName = None):
        ...
        attractorDF.index = [
            self.geneList[temp] for temp in self.nodePositions
        ]
        attractorDF = attractorDF.loc[:,
                                    attractorDF.columns.isin(distanceDF.decider)]
        ...
        if isinstance(allAttractors, list):
            ...
            attractorDF2 = attractorDF2.loc[attractorDF2.sum(
                axis=1) != len(allAttractors), :]
        else:
            attractorDF2 = attractorDF
            allAttractors = distanceDF.decider.value_counts()[:numberOfAttractorsToShow].index.tolist(
            )  # top n attractors where n = numberOfAttractorsToShow, default = 2
        ...
        if attractorDF2.shape[1] > 1:        
            ax = sns.clustermap(attractorDF2,
                                cmap=cmap,
                                linewidth=0.3,
                                yticklabels=True,
                                xticklabels=True,
            ...
            if outputFileName is not None:
                ...
                ax.savefig(str(network) + "_multiple_attr_heatmap.pdf",bbox_inches='tight',pad_inches=0.01)
            plt.clf()
        else:
            ax = sns.heatmap(attractorDF2,vmin=0,vmax=1,cmap=cmap,xticklabels=False,cbar_kws={"ticks": [0, 1]})
            ax.set(ylabel="Genes")
            if outputFileName is not None:
                ax.savefig(outputFileName,bbox_inches='tight',pad_inches=0.01)
            else:
                ax.figure.savefig(str(network) + "_one_attr_heatmap_mgp.pdf",bbox_inches='tight',pad_inches=0.01)
            plt.clf()
        return (attractorDF, ax)

    def makeAttractorAnalysisPlots(self, plottingData, distanceDF, allAttractors=False, numberOfAttractorsToShow = 2, cmap = "colorblind", makeFrequencyPlots = True, freqplotsFile = "", makeUMAP=True, umapFile = ""):
        if makeFrequencyPlots:
            #Make attractor frequency plots
            sns.set_context("talk")
            plt.figure(figsize=(5, 5))
            plottingData.loc[:, "Attractors"] = distanceDF['decider'].astype(
                "category")
            plottingData.loc[:, "Attractors"].cat.categories = [
                g for g in range(
                    len(plottingData.loc[:, "Attractors"].cat.categories))
            ]
            if isinstance(allAttractors, list):
                plottingData.Attractors = [
                    temp if temp in allAttractors else "Other"
                    for temp in plottingData.Attractors
                ]
                palette = {}
                attrColors = {
                    key: value
                ...
                plottingData.Attractors = [
                    temp if temp in plottingData.Attractors.value_counts()
                    [:2].index.tolist() else "Other"
                    for temp in plottingData.Attractors
                ]
                allAttractors = plottingData.Attractors.value_counts(
                )[:numberOfAttractorsToShow].index.tolist()
                palette = {}
                attrColors = {
                    key: value
                    for key, value in zip(
                ...
                for attr in set(plottingData.Attractors):
                    if attr in allAttractors:
                        ...
                        palette[attr] = (0.75, 0.75, 0.75)
            histData = plottingData.groupby(
                by=['Sample', 'Condition'])['Attractors'].value_counts(
                    normalize=True).mul(100).rename('Percent').reset_index()
            ax = sns.catplot(data=histData,
                            y='Sample',
                            x='Percent',
                            hue='Attractors',
                            col="Condition",
                            ...
                            kind='bar',
                            sharey=False)
            ax.set(xlabel="Percentage of Cells\nMapped to Attractors",
            ...
            if freqplotsFile is "":
                ...
                ax.figure.savefig(freqplotsFile,
                                bbox_inches='tight',
                                pad_inches=0.01)
                plt.show()
                plt.clf()
        ...
        if makeUMAP:
            ...
            plt.figure(figsize=(5, 5))
            ax = sns.scatterplot(data=plottingData,
                                edgecolor="black",
                                linewidth=1,
                                x="UMAP dimension 1",
                                y="UMAP dimension 2",
            ...
            if umapFile is not "":
                ...
                figl.savefig(umapFile + "_legend.pdf",
                bbox_inches='tight',
                pad_inches=0.01)
                plt.clf()
        else:
