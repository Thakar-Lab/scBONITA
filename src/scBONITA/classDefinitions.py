    ...
    for edge in sif:
        ...
        if (node1 == node2):
            ...
            if int(edge[1]) == 1:
                ...
                act="u"
            G.add_edge(str(edge[0]), str(edge[2]), signal=int(edge[1]), activity=act, interaction=act)
...
class ruleMaker:
    ...
    def makeToolBox(self, graph):
        ...
        #MAKE TYPES
        creator.create("FitnessMin", base.Fitness, weights=weightTup) # make a fitness minimization function #the objective function has to be MINIMIZED
        creator.create("individual", list, fitness=creator.FitnessMin)    # create a class of individuals that are lists of floats
    ...
    def getPreds(self, geneList, graph):
        ...
        for n in nodes:
            ...
            for p in preds:
                ...
                corDict[p]=sklearn.metrics.normalized_mutual_info_score(self.binMat[n_index,:].todense().tolist()[0], self.binMat[p_index,:].todense().tolist()[0], average_method="arithmetic")
                # print(n, p, corDict[p])
                corHist.append(corDict[p])
        self.corHist=corHist

    def __init__(self, graph, removeSelfEdges=False, restrictIncomingEdges=True, maxIncomingEdges=3, groundTruth=False, graphName=""):
        if maxIncomingEdges < 3:
            print("The maximum number of incoming edges has been set to less than 3. Meaningless results await you.")
        ...
        self.nodePositions = [self.geneList.index(node) for node in nodeList] # node positions in geneList
        ...
        nodeDict={} #i dentifies names of nodes with their index in the node list- provide name, get index
        possibilityLister=[]
        possibilityInverter=[]
        succnum=[]
        ...
        for i in range(0,len(nodeList)):
            predecessors_temp=list(graph.predecessors(nodeList[i])) # get NAMES of predecessors of node as documented in the original graph
            successors_temp=list(graph.successors(nodeList[i])) # get NAMES of successors of node as documented in the original graph
            ...
            nodeData=self.binMat[self.nodePositions[i], :].todense().tolist()[0] #find binarized expression data for node "i"
            predCorr_temp=[] #temporarily store correlations between node "i" and all its predecessors
            for k in predecessors_temp:
                predIndex = self.geneList.index(k) #find index of predecessor in the geneList from the data
                predData=self.binMat[predIndex, :].todense().tolist()[0] #find binarized expression data for predecessor "k"
                mi, pvalue = spearmanr(nodeData,predData)
                if np.isnan(mi):
                    predCorr_temp.append(0)
                else:    
                    predCorr_temp.append(mi) #store the calculated correlation
            predecessors_final=sorted(zip(predecessors_temp, predCorr_temp), reverse=True, key=lambda corrs: corrs[1])[:3] #find the top predecessors of the node "i"
            self.rvalues.append(sorted(predCorr_temp, reverse=True)[:3]) # stores the correlations
            ...
            for parent in predecessors_final:
                if 'interaction' in list(graph[parent[0]][nodeList[i]].keys()):
                    ruleGraph.add_edge(parent[0], nodeList[i], weight=parent[1], activity=graph[parent[0]][nodeList[i]]['interaction'])
                if 'signal' in list(graph[parent[0]][nodeList[i]].keys()):
                    ruleGraph.add_edge(parent[0], nodeList[i], weight=parent[1], activity=graph[parent[0]][nodeList[i]]['signal'])
            ...
            withNones = zip([nodeList.index(corr_tuple[0]) for corr_tuple in predecessors_final], itertool.repeat('empty'))
            ...
            for sequence in possibilities:
                ...
                for node in sequence:
                    ...
                    if 'interaction' in list(graph[nodeList[node]][nodeList[i]].keys()):
                        if graph[nodeList[node]][nodeList[i]]['interaction']=='a':
                            ...
                            if graph[nodeList[node]][nodeList[i]]['interaction']=='i':
                                ...
                                if graph[nodeList[node]][nodeList[i]]['interaction']=='u':
                                    print("Unknown interaction type, assigning activation...")
                                    activity.append(False)
                                else:
                                    if graph[nodeList[node]][nodeList[i]]['interaction']=='g':
                                        print("Group edge/interaction type, assigning activation...")
                                        activity.append(False)
                                    else:
                                        print("Unknown interaction, assigning activation...")
                                        activity.append(False)
                        ...
                        if graph[nodeList[node]][nodeList[i]]['signal']=='a':
                            ...
                            if graph[nodeList[node]][nodeList[i]]['signal']=='i':
                                ...
                                if graph[nodeList[node]][nodeList[i]]['signal']=='u':
                                    ...
                                    if graph[nodeList[node]][nodeList[i]]['signal']=='g':
                                        print("Group edge/interaction type, assigning activation...")
                                        activity.append(False)
                                    else:
                                        print("Unknown interaction, assigning activation...")
                    ...
                    if not 'interaction' in list(graph[nodeList[node]][nodeList[i]].keys()) and not 'signal' in list(graph[nodeList[node]][nodeList[i]].keys()):
                        print("Group edge/interaction type, assigning activation...")
                        activity.append(False)
    ...
    def NP(self, individual, model, cells, params, KOs, KIs, scSyncBoolC):
        ...
        binMatC3=np.transpose(np.array(copy.deepcopy(binMatC1), order="C",dtype=np.intc))
        #print(binMatC3)
        #print(binMatC3.shape)
        binMatCPointer=ctypes.c_void_p(binMatC3.ctypes.data) # put input array as C pointer
        ...
        vals=np.full(shape=(self.maxNodes, self.params.simSteps, self.maxSamples), fill_value=2, dtype=np.intc, order='C') # initiate output array - sim data is nodes * sim steps * cells. Max sim steps hard coded to 200
        ...
        scSyncBoolC(valsubmit,nodeIndividual1, indLen1, nodeNum1, andLenList1, individualParse1, andNodes1, andNodeInvertList1, simSteps1, knockouts1, knockins1, lenSamples, binMatCPointer, nodePositionsCPointer)
    ...
    def evaluateByNode(self, individual, cells, model,  sss, params, KOlist, KIlist,scSyncBoolC, localSearch=False):
        ...
        boolValues=self.NP(individual, model, self.params.cells, self.params, KOlist, KIlist, scSyncBoolC)
        ...
        return self.calculateErrorFromSimData(simData=boolValues, localSearch=localSearch, model=model)
    ...
    def calculateErrorFromSimData(self, simData, localSearch, model):
        ...
        for node in list(model.nodePositions):
            ...
            for simStep in range(self.params.simSteps-1, self.params.simSteps): #iterate over the last three steps of the simulation trajectory
                for k in range(0, len(model.sampleList)): #iterate over all cells, ie, over all starting states
            ...
            error_k = [abs(tempCell - tempAttr) for tempCell, tempAttr in zip(cellInitValue, simAvg_k)]
        ...
        if localSearch:
            ...
            allNodeErrors = [sum(allNodeErrors[i, :])/len(model.sampleList) for i in range(0, len(model.nodePositions))]
            ...
            allCellErrors = [float(temp)/len(model.sampleList) for temp in allCellErrors]
    ...
    def calculateErrorFromSimData2(self, simData, localSearch, model):
        if localSearch:
            ...
            for node in self.nodePositions:
                print(node)
                d_ij = self.binMat.tocsr()[node, range(0, len(model.sampleList))].toarray()[0] #value of node 'node' in all samples
                o_ij = simData[node, self.params.simSteps-1, range(0, len(model.sampleList))] #value of node in the
                print(simData[node,:, :])
                d_ij = list(d_ij)
                o_ij = list(o_ij)
                errorTemp=[abs(tempCell - tempAttr) for tempCell, tempAttr in zip(d_ij, o_ij)]
                #errorTemp=abs(d_ij - o_ij)
                print('error_k '+str(list(errorTemp)) + '\t' + str((d_ij))+ '\t' + str((o_ij)))
                #print(list(errorTemp))
                if errorTemp[node] != 0:
                    errorTemp = [float(60000)] * len(model.nodePositions) #[float(10000)] * len(model.nodePositions)
            ...
            for k in range(0, len(model.sampleList)): #iterate over all cells, ie, over all starting states
                ...
                for node in range(0, len(self.nodePositions)):
                    ...
                    o_ij = simData[node, self.params.simSteps-1, k] #value of node in the final step of simulation starting from k
                    errorTemp=abs(d_ij - o_ij)
                    print('error_k '+str(errorTemp) + '\t' + str(d_ij)+ '\t' + str(o_ij))
    ...
    def calculateImportanceScoreFromSimData(self, simData1, simData2):
        ...
        allCellErrors=[]
        for k in range(0, len(self.sampleList)): #iterate over all cells, ie, over all starting states
            ...
            error_k = [abs(tempCell1 - tempCell2) for tempCell1, tempCell2 in zip(simAvg_k1, simAvg_k2)]
            allCellErrors.append(error_k)
        allCellErrors = [sum(temp) for temp in allCellErrors]
    ...
    def varOrAdaptive(self, population, toolbox, lambda_, cxpb, mutpb, genfrac, mutModel):
    ...
    def selectMutNode(self, errors):
        ...
        normerrors=[1.*error/ np.sum(errors) for error in errors]# normalize errors to get a probability that the node  is modified
    ...
    def mutFlipBitAdapt(self, indyIn, genfrac, mutModel):
        ...
        if np.sum(errors)<.05*errorNodes or errorNodes==0:
        ...
        if model.andLenList[focusNode]>1:
            ...
            if len(model.possibilityList[focusNode])>3 and random()<mutModel:
                ...
                while len(temppermup)<3:
                    ...
                    tempsum=sum(rvals)
                    if tempsum==0:
                        addNoder=randint(0, len(rvals)-1) #int(math.floor(random()*len(upstreamAdders)))
                        print(addNoder)
                    else:
                        recalc= np.cumsum([1.*rval/tempsum for rval in rvals])
                        print(recalc)
    ...
    def eaMuPlusLambdaAdaptive(self, scSyncBoolC, graph, verbose=True):
        ...
        modellist.append([[(modeler[0].size), list(modeler[0].nodeList), list(modeler[0].individualParse), list(modeler[0].andNodeList) , list(modeler[0].andNodeInvertList), list(modeler[0].andLenList), list(modeler[0].nodeList), dict(modeler[0].nodeDict)] for modeler in population])
        ...
        for ind in population:
            if np.sum(ind.fitness.values)< .01*len(ind.fitness.values):
                breaker=True
        if breaker:
        ...
        for gen in range(1, ngen+1):
            ...
            offspring = self.varOrAdaptive(population, toolbox, lambda_, cxpb,  mutpb, (1.*gen/ngen),mutModel)
            # Evaluate the individuals with an invalid fitness
            invalid_ind = [ind for ind in offspring if not ind.fitness.valid]
            fitnesses=[self.evaluateByNode(indy[1], self.params.cells, indy[0], self.nodePositions, self.params, KOlist, KIlist,scSyncBoolC) for indy in invalid_ind]
            #fitnesses=[indy[0].evaluateByNode(indy[1], indy[0].params.cells, indy[0], indy[0].nodePositions, indy[0].params, KOlist, KIlist,scSyncBoolC) for indy in invalid_ind]
    ...
    def checkNodePossibilities(self, node, indy, newSSS, cellNum, model, params, KOlist, KIlist, scSyncBoolC):
        #print(node)
        ...
        for i in range(1,2**(end-start)):
            ...
            currentsumtemp=self.evaluateByNode(tempultimate, self.params.cells, model, model.nodePositions, self.params, KOlist, KIlist, scSyncBoolC, localSearch=True) #self.evaluateByNode(tempultimate, self.params.cells, self, self.nodePositions, self.params, KOlist, KIlist, scSyncBoolC, localSearch=True) #gives one error per cell
            #print(currentsumtemp)
            currentsum=currentsumtemp[node] # save the error found #subset complete error to
        ...
        truth=equivs[0]
        print("Node\t" + str(node) + "\t" + 'indErrors\t'+str(indErrors) + '\t' + "Minny\t" + str(minny)+ '\t' + "Len_equivs\t" + str(len(equivs)) + "Equivs\t" + str(equivs))
    ...
    def writeModel(self, individual, model):
        ...
        for i in range(0,len(model.nodePositions)):
            addString=addString+model.writeNode(i,individual[model.individualParse[i]:model.individualParse[i+1]], model)
            addString=addString+'\n'
        return addString[:-1]
    ...
    def writeModel_BoolNet(self, individual, model):
        ...
        #for i in range(0,len(self.nodeList)):
        for i in range(0,len(model.nodePositions)):
            addString=addString+model.writeNode_BoolNet(i,individual[model.individualParse[i]:model.individualParse[i+1]], model)
            addString=addString+'\n'
        return addString[:-1]
    ...
    class Params:
        ...
        def simParams(self, mutModel=0.25, cells=1, samples=1, generations=10, popSize=24, mu=10, lambd=24, iters=100, genSteps=100, simSteps=100,crossoverProb=0.1, mutationProb=0.9, bitFlipProb=0.5):
            ...
            self.crossoverProb=crossoverProb # prob of crossing over a particular parent
...
class singleCell(ruleMaker):
    ...
    def __init__(self, dataName, sep):
        ...
        self.geneList, self.sampleList, self.expMat = data[1:,0], data[0,1:], sparse.csr_matrix(data[1:,1:].astype("float"))
        ...
        self.binMat=preprocessing.binarize(self.expMat, threshold=0.001, copy=False) #if data >=0: binMat = 1
    ...
    def find_pathways_from_gmt_LEGACY(self, gmtFile, minOverlap=4):
        ...
        for key in GMTdict.keys():
            if len(pathwayGenes.intersection(GMTdict[key]))>=minOverlap: # ensure there are at least minOverlap nodes in both pathway and detected genes
                print(key)
    ...
    def find_pathways(self, organism="hsa", minOverlap=20, writeGraphml=True, predefinedList=[]):
            ...
            for x in list(k.pathwayIds):
                ...
                try:
                    ...
                    if len(sif) < 1:
                        ...
                        if test >= minOverlap:
                            print("Pathway: ", x, " Overlap: ", test, " Edges: ", len(sif))
                            G=sif_to_digraph(sif=sif, pathwayGenes=pathwayGenes)
                            self.pathwayGraphs[x]=G
                            if writeGraphml:
                                nx.write_graphml(G, x+"_processed.graphml", infer_numeric_types=True)
                except:
                    next
        else:
    ...
    def add_wikipathways(self, minOverlap=20, writeGraphml=True, removeSelfEdges=False):
        ...
        for x in list(glob.glob('*[0-9].graphml')):
            ...
            if test >= minOverlap:
                ...
                #To do: remove complexes, remove dependences of a node on complexes that include that node (which is a form of self-loop)
                self.pathwayGraphs[x]=G
                print("Edges after processing:", len(G.edges())," Overlap: ", len(set(G.nodes()).intersection(pathwayGenes)))
                if writeGraphml:
                    nx.write_graphml(G, x+"_processed.graphml", infer_numeric_types=True)
    ...
    def makemetaNetwork(self):
        if not hasattr(self, "pathwayGraphs"):
            ...
            self.metaNetwork.remove_nodes_from([n for n in self.metaNetwork if n not in list(largest)])
            print("The meta network has ", self.metaNetwork.number_of_nodes(), " nodes.")
    ...
    def calcImportance(self, individual, params, model, sss, knockoutLists, knockinLists, scSyncBoolC):
        ...
        for node in self.nodePositions:
            ...
            KO_temp[node]=1
            print(KO_temp)
            boolValues1=self.NP(individual, model, self.params.cells, self.params, KO_temp, knockinLists, scSyncBoolC) #knocked out
            KI_temp=copy.deepcopy(self.knockinLists)
            KI_temp[node]=1
            print(KI_temp)
            boolValues2=self.NP(individual, model, self.params.cells, self.params, knockoutLists, KI_temp, scSyncBoolC) #knocked in
            importanceScores.append(self.calculateImportanceScoreFromSimData(boolValues1, boolValues2))
    ...
    def scorePathway(self, RAs, pathImportances, pathname):
        ...
        for node in pathImportances:
            ...
            nodeData=self.binMat[self.geneList.index(node), range(0, len(self.sampleList))].todense().tolist()
            ...
            print(CVdict[node])
            impactScore[node] = abs(float(RAs[node]))*float(pathImportances[node])*CVdict[node]
        ...
        n, bins, patches = plt.hist(x=randomScores, bins='auto', color='#0504aa',
        ...
        plt.text(meaner, y_max-15, 'Mean='+str(round(meaner, 2))+'\n Std.dev='+ str(round(stdev,2)) + '\n Pathway score=' + str(round(score,2)), bbox=dict(facecolor='red', alpha=0.75))
        plt.savefig(pathname[:8]+"_impact_scores_hist.png", format="png", bbox_inches='tight')
        plt.close()
    ...
    def run_pathway_analysis_from_outs(self, contrast, conditions, delimiter):
        ...
        for contrasts in contrastList:
            ...
            cells_condition1= conditions.loc[conditions[str(contrasts[0])] == 1, list(conditions.columns)[0]]
            ...
            cells_condition2= conditions.loc[conditions[str(contrasts[1])] == 1, list(conditions.columns)[0]]
            cells_condition2 = list(cells_condition2)
            print(len(cells_condition1))
            index_condition1=[self.sampleList.index(i) for i in set(cells_condition1).intersection(set(self.sampleList))]
            index_condition2=[self.sampleList.index(i) for i in set(cells_condition2).intersection(set(self.sampleList))]
            ...
            for pathname in list(self.pathwayGraphs.keys()):
                ...
                if os.path.exists(pathname[:-8]+"_IS.graphml") or os.path.exists(pathname[:-8]+".graphml_processed.graphml_importanceScores.csv"):
                    """
                    ...
                    """
                    if os.path.exists(pathname[:-8]+".graphml_processed.graphml_importanceScores.csv"):
                        print(pathname[:-8]+".graphml_processed.graphml_importanceScores.csv")
                        paNetTemp = nx.read_graphml(pathname[:-8]+".graphml_processed.graphml")
                        # get nodeScores
                        nodeScoresDF = pd.read_csv(pathname[:-8]+".graphml_processed.graphml_importanceScores.csv")
                        nodeScoresDF.index = list(nodeScoresDF.Node)
                        if "Strat3_IS" in nodeScoresDF.columns:
                            # add impact score as attribute to graph
                            nodeScores = dict(zip(list(nodeScoresDF.index), list(nodeScoresDF.loc[:, "Strat3_IS"])))
                            nx.set_node_attributes(paNetTemp, values=nodeScores, name='Strat3_IS')
                            # add obsERS as attribute to graph
                            obsERS = dict(zip(list(nodeScoresDF.index), list(nodeScoresDF.loc[:, "ObsERS"])))
                            nx.set_node_attributes(paNetTemp, values=obsERS, name='Observed ERS')
                            # add maxERS as attribute to graph
                            maxERS = dict(zip(list(nodeScoresDF.index), list(nodeScoresDF.loc[:, "MaxERS"])))
                            ...
                            for node in list(nodeScoresDF.index):
                                node_index=self.geneList.index(node)
                                binexpression_condition1[node] = np.mean(self.binMat2[node_index, index_condition1].tolist())
                                binexpression_condition2[node] = np.mean(self.binMat2[node_index, index_condition2].tolist())
                                expression_condition1[node] = np.mean(self.expMat2[node_index, index_condition1].tolist())
                                expression_condition2[node] = np.mean(self.expMat2[node_index, index_condition2].tolist())
                                print([expression_condition1[node], expression_condition2[node]])
                                RA[node] = expression_condition1[node] - expression_condition2[node]
                                if expression_condition1[node] > expression_condition2[node]:
                                    upreg_condition1[node] = True
                                else:
                                    upreg_condition1[node] = False
                            nodeScoresDF[str(contrasts[0])] = nodeScoresDF['Node'].map(expression_condition1)
                            nodeScoresDF[str(contrasts[1])] = nodeScoresDF['Node'].map(expression_condition2)
                            nodeScoresDF["BIN_"+str(contrasts[0])] = nodeScoresDF['Node'].map(binexpression_condition1)
                            nodeScoresDF["BIN_"+str(contrasts[1])] = nodeScoresDF['Node'].map(binexpression_condition2)
                            nodeScoresDF[str("Upregulated_in_"+str(contrasts[0]))] = nodeScoresDF['Node'].map(upreg_condition1)
                            # add RA as attribute to graph
                            nx.set_node_attributes(paNetTemp, values=RA, name='relativeAbundance')
                            nx.set_node_attributes(paNetTemp, values=expression_condition1, name=str(contrasts[0]))
                            nx.set_node_attributes(paNetTemp, values=expression_condition2, name=str(contrasts[1]))
                            nx.set_node_attributes(paNetTemp, values=upreg_condition1, name=str("Upregulated_in_"+str(contrasts[0])))
                            z_scores=[]
                            modRA = {}
                            for node in list(nodeScoresDF.index):
                                modRA[node] = abs(RA[node])
                            # iterate over comparisons for each pathway and calculate z score
                            zscore, impactScore, CVdict = self.scorePathway(modRA, nodeScores, pathname[:-8])
                            z_scores.append(zscore)
                            pvals = scipy.stats.norm.sf(z_scores) # calculate p value
                            #print(pvals)
                            pvalDict[str(pathname)]=[pvals, str(CVdict), str(zscore), str(sum(impactScore.values())), str(mean(impactScore.values()))]
                            ...
                            if pvalDict[str(pathname)][0] < 0.05:
                                nx.write_graphml_lxml(paNetTemp, pathname[:-8]+"_IS_"+"_vs_".join(contrasts)+".graphml")
                            nodeScoresDF.to_csv(pathname[:-8]+".graphml_processed.graphml_importanceScores.csv", index = False)
                            if sum(nodeScoresDF[str("Upregulated_in_"+str(contrasts[0]))]) > len(list(nodeScoresDF.index))/2:
            ...
            for key,value in pvalDict.items():
                ...
                pvalDict[key] = [key, str(getPathwayName(key[:8])), str(value[0][0]),"_vs_".join(contrasts), str(overallUpreg[key]), str(value[1]), str(value[2]), str(value[3]), str(value[4])]
                #fh.write("\n")
            #fh.close()
            pvalDF = pd.DataFrame.from_dict(pvalDict, orient="index")
            pvalDF.columns = ["Pathway ID", "Pathway Name", "P value", "Contrast", str("Upregulated_in_"+str(contrasts[0])), "CVdict", "zscore", "impactScore", "meanNodewiseImpactScore"]
    ...
    def scoreNodes(self, graph):
        ...
        #Local search
        outputs=[self.checkNodePossibilities(node, out2, model.sampleList, model.params.cells, model, model.params, model.knockoutLists, model.knockinLists,scSyncBoolC) for node in range(0, len(model.nodePositions))]
        equivs=[]
        individual=[]
        devs=[]
        localErrors=[]
        ...
        scores1=self.calcImportance(bruteout2, model.params, model, model.sampleList, model.knockoutLists, model.knockinLists, scSyncBoolC)
    ...
    def inherit(self, graph, removeSelfEdges=False, restrictIncomingEdges=True, maxIncomingEdges=3, groundTruth=False, graphName=""):
        super().__init__(graph, removeSelfEdges, restrictIncomingEdges, maxIncomingEdges, groundTruth, graphName)
...
def supplementBONITA(pathwayGraphs):
    ...
    fh.write("".join(["Pathway ID,", "Pathway Name,", "Length of shortest longest path,", "Number of nodes in pathway,","Number of source nodes,","Mean out-degree of source nodes", "\n"]))
    ...
    for name, path in pathwayGraphs.items():
        ...
        fh.write("".join([str(name), ",", str(pathName), ",", str(tempMax), ",", str(numberNodes), ",", str(len(sourceNodes)),",", str(sourceOutDeg_mean), "\n"]))
    fh.close()
...
def pipeline(dataName="", sep=",", subpopFile=""):
    ...
    scTest.find_pathways(minOverlap=10, predefinedList=[
    ...
    scTest.binMat.resize((scTest.maxNodes, scTest.maxSamples)) #restrained to 1000 genes and 1000 samples for testing simulation code, so that I can have a 200 step simulation
    print(scTest.binMat.A[0:10,0:10])
    print(scTest.binMat.shape)
