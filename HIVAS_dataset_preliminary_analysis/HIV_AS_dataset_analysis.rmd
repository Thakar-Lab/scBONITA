...
setwd("D:\\GitHub\\fuzzyRNA\\clustering\\")
...
age = scRNA@meta.data$batchid
age = sapply(
  age,
  FUN = function(x) {
    return(patient_data$Age[patient_data$ParticipantId == x])
  }
)
names(age) = rownames(scRNA@meta.data)
scRNA <- AddMetaData(object = scRNA,
                     metadata = age,
                     col.name = "age")

cohort = scRNA@meta.data$batchid
cohort = sapply(
  cohort,
  FUN = function(x) {
    return(patient_data$Cohort[patient_data$ParticipantId == x])
  }
)
names(cohort) = rownames(scRNA@meta.data)
scRNA <- AddMetaData(object = scRNA,
                     metadata = cohort,
                     col.name = "cohort")

chol = scRNA@meta.data$batchid
chol = sapply(
  chol,
  FUN = function(x) {
    return(patient_data$chol[patient_data$ParticipantId == x])
  }
)
...
PlaqueScore = scRNA@meta.data$batchid
PlaqueScore = sapply(
  PlaqueScore,
  FUN = function(x) {
...
This is where we anonymize the participant IDs. I repeat:

## THE PATIENT IDS ARE STRICTLY CONFIDENTIAL.

```{r}

# Add anonymized participant IDs to the metadata

anonIDs = c("VP090" = "Participant_1",
            "VP029" = "Participant_2",
            "VP086" = "Participant_3",
            "VP087" = "Participant_4",
            "VP058" = "Participant_5",
...
scRNA2@meta.data$ParticipantID =  plyr::revalue(x = as.character(scRNA2@meta.data$batchid), replace = anonIDs, warn_missing = TRUE)
...
# GSEA and Enricher for AS+ vs AS-, for each subpopulation separately (Lauren). Also modified to do comparison for only scbonita networks
...
# Get cluster centroids of all clusters (for Lauren)
