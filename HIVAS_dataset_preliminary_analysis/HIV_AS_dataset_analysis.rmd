...
#setwd("") # you should update this appropriately
...
for (i in levels(as.factor(cellCounts$res.1.5))){
  ...
  print(c("Cluster Number: ", i))
  print(c("Cluster Name: ", unique(cellCounts$ClusterNames[cellCounts$res.1.5 == i])))
  print(c("Pvalue, t-test: ", t.test(a, b)$p.value))
  print(c("Pvalue, wilcox: ", wilcox.test(a, b)$p.value))
  print("****")
}
...
```{r}
scRNA2.markers$cluster = unlist(lapply(scRNA2.markers$cluster, function(x){return(as.numeric(unique(scRNA2@meta.data$res.1.5[scRNA2@meta.data$ClusterName==x])))}))
...
imm_t2g = clusterProfiler::read.gmt("c7.all.v7.1.symbols.gmt") #immsigdb gmt file/gene sets
bonita_t2g = clusterProfiler::read.gmt("graphs_to_gmt_scBonita.gmt") #gmt for scBonita/KEGG networks
...
for (cluster in 0:15){
  ...
  write.csv(temp_markers, paste0("cluster_",as.character(cluster),"_ASmarkers.csv"))
...
kable(as.data.frame(enricher(DEgenes, TERM2GENE = kegg_t2g))) #enricher-kegg results
```
...
```{r}
# For "majority" cell types, identified by looking at output and bar plot from CIBERSORT
cibersort_majority = c("0" = "B cells naive - 1", "1" = "T cells CD8 - 1", "2" = "T cells CD4 - 1", "3" = "T cells CD8/CD4/CD4 naive", "4" = "T cells CD8 naive/T cells CD8/ RBCs", "5" = "B cells naive - 2", "6" = "T cells naive", "7" = "T cells CD8 - 2", "8" = "T cells CD8 - 3", "9" = "NK cells resting", "10"  = "T cells CD8 - 4", "11" = "T cells CD8/NK cells resting", "12" = "T cells CD4 memory resting/ T cells CD8", "13" = "B cells memory", "14" = "Monocytes", "15" = "NK cells activated")
